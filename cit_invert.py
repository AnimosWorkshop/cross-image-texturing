...
from src.cit_utils import image_to_tensor, tensor_to_image
...
def invert_lidor(num_steps, image_tensor, prompt, device="cuda:0"):
    """
    @return: a 3D tensor of shape (x,y,z).
    """
    model = Preprocess(device=device, sd_version="1.5")
    image_pil = tensor_to_image(image_tensor)
    image = model.prepare_image(image_pil)
    inverted_latents = model.invert_latents(num_steps=num_steps, image=image) # only one latent, since we provide only one image
    return inverted_latents[0]
    # TODO use this to invert_DDIM and there you go: you got the inverted latents.
...
def get_inverted_views(tex_app_path, mesh_path_app, num_steps, prompt_for_inversion, save_path, camera_azims=[-180, -135, -90, -45, 0, 45, 90, 135], camera_centers=None) -> list:
	...
	for i in range(len(uvp_app.cameras)):
		...
		inverted_x = invert_lidor(num_steps=num_steps, image_tensor=image_tensor, prompt=prompt_for_inversion)
		demo(prompt=prompt_for_inversion, latent=inverted_x) # TODO remove this line
		exit(0)
		latents_app.append(inverted_x)
	...
	torch.save(latents_app, save_path)
